...
public final class CloudQueueClientTests extends QueueTestBase {
    ...
    public void testQueueClientConstructor() throws URISyntaxException, StorageException {
        StorageCredentials credentials = this.httpAcc.getCredentials();
        URI baseAddressUri = new URI(this.httpAcc.getQueueEndpoint().toString());
    ...
    public void testQueueClientConstructorInvalidParam() throws URISyntaxException, StorageException {
        StorageCredentials credentials = this.httpAcc.getCredentials();
        try {
            CloudQueueClient queueClient1 = new CloudQueueClient(null, credentials);
            Assert.fail();
        }
        ...
        try {
            ...
            CloudQueueClient queueClient1 = new CloudQueueClient(new URI(name.toString()), credentials);
            Assert.fail();
        }
    ...
    public void testListQueuesSmallNumber() throws URISyntaxException, StorageException {
        ...
        for (CloudQueue queue : this.qClient.listQueues()) {
            initialCount++;
        }
        ...
        for (int i = 0; i < 25; i++) {
            CloudQueue q = new CloudQueue(AppendQueueName(this.httpAcc.getQueueEndpoint(), UUID.randomUUID().toString()
                    .toLowerCase()), this.qClient);
        ...
        for (int i = 0; i < 25; i++) {
            CloudQueue q = new CloudQueue(AppendQueueName(this.httpAcc.getQueueEndpoint(), perfix
                    + UUID.randomUUID().toString().toLowerCase()), this.qClient);
        ...
        count = 0;
        for (CloudQueue queue : this.qClient.listQueues(perfix, QueueListingDetails.METADATA, null, null)) {
            count++;
    ...
    public void testListQueuesAndListQueuesSegmentedLargeNumber() throws URISyntaxException, StorageException {
        ...
        if (count < totalLimit) {
            ...
            for (int i = 0; i < totalLimit - count; i++) {
                ...
                CloudQueue q = new CloudQueue(AppendQueueName(this.httpAcc.getQueueEndpoint(),
                        String.format("listqueue" + sub.replace(",", ""))), this.qClient);
                q.createIfNotExist();
            }
        }
        ...
        ResultSegment<CloudQueue> segment = this.qClient.listQueuesSegmented();
    ...
    public void testTimeout() throws URISyntaxException, StorageException {
        Assert.assertTrue(this.qClient.getTimeoutInMs() == 30 * 1000);
        qClient.setTimeoutInMs(60 * 1000);
        Assert.assertTrue(this.qClient.getTimeoutInMs() == 60 * 1000);
    }
