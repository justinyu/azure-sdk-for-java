...
public class CloudQueueTests extends QueueTestBase {
    ...
    public void testQueueClientConstructor() throws URISyntaxException, StorageException {
        ...
        String baseAddress = AppendQueueName(this.qClient.getEndpoint(), queueName);
        CloudQueue queue1 = new CloudQueue(baseAddress, this.qClient);
        Assert.assertEquals(queueName, queue1.getName());
        Assert.assertTrue(queue1.getUri().toString().endsWith(baseAddress));
        Assert.assertEquals(this.qClient, queue1.getServiceClient());

        CloudQueue queue2 = new CloudQueue(new URI(AppendQueueName(this.qClient.getEndpoint(), queueName)),
                this.qClient);

        Assert.assertEquals(queueName, queue2.getName());
        Assert.assertEquals(this.qClient, queue2.getServiceClient());

        CloudQueue queue3 = new CloudQueue(queueName, this.qClient);
        Assert.assertEquals(queueName, queue3.getName());
        Assert.assertEquals(this.qClient, queue3.getServiceClient());
    }
    ...
    public void testGetMetadata() throws URISyntaxException, StorageException {
        ...
        Assert.assertEquals(this.queue.getMetadata().get("ExistingMetadata"), "ExistingMetadataValue");
        Assert.assertTrue(this.queue.getMetadata().containsKey("ExistingMetadata"));
    ...
    public void testQueueCreate() throws URISyntaxException, StorageException {
        String queueName = UUID.randomUUID().toString().toLowerCase();
        CloudQueue queue = this.qClient.getQueueReference(queueName);
    ...
    public void testDeleteQueueIfExists() throws URISyntaxException, StorageException {
        ...
        final CloudQueue queue = this.qClient.getQueueReference(queueName);
    ...
    public void testClearMessages() throws URISyntaxException, StorageException, UnsupportedEncodingException {
        final CloudQueue queue = this.qClient.getQueueReference(UUID.randomUUID().toString().toLowerCase());
        queue.create();
    ...
    public void testDeleteMessage() throws URISyntaxException, StorageException, UnsupportedEncodingException {
        String queueName = UUID.randomUUID().toString().toLowerCase();
        CloudQueue newQueue = this.qClient.getQueueReference(queueName);
        newQueue.create();
    ...
    public void testQueueDownloadAttributes() throws URISyntaxException, StorageException,
        ...
        final CloudQueue queue1 = this.qClient.getQueueReference(queueName);
        queue1.create();
        ...
        final CloudQueue queue2 = this.qClient.getQueueReference(queueName);
